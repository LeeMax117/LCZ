...
if finetune_ckpt and is_training:
    ...
    if is_training:
        ...
        try:
            ...
            print('create new training status json file')
            ...
            print('normalized para has been stored in %s'%json_path_train)
    else:
        ### load validation status data from json file
        try:
            with open(json_path_valid, 'r') as file:
                valid_data_paras = json.load(file)
                raw_data.set_normalize_para(valid_data_paras['average'], valid_data_paras['standard'])
        except (IOError) as e:
            print('create new validation status json file')
            ...
            with open(json_path_valid, 'w') as f:
                ...
                dict_to_dump['standard'] = raw_data.standard
                json.dump(dict_to_dump, f)
            print('normalized para has been stored in %s'%json_path_valid)
...
if is_training:
    ...
    while True:
        ...
        _, acc, summary, loss, l2_loss_value, total_loss_value, steps = sess.run(
            [train_step, accuracy, summary_op, cross_entropy, l2_loss, total_loss, increment_op],
